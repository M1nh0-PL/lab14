...
W treści zadania wymagano, aby phpMyAdmin był dostępny na porcie 6001 oraz umożliwiał logowanie do serwera MySQL i tworzenie testowej bazy danych.

Aby phpMyAdmin mógł komunikować się z serwerem MySQL, oba kontenery muszą znajdować się w tej samej sieci Docker.

Z tego powodu phpMyAdmin został podłączony do sieci backend, w której znajduje się również kontener MySQL.

Dzięki temu phpMyAdmin może korzystać z nazwy hosta:

```text
mysql
```

i nawiązywać połączenie z bazą danych bez konieczności publikowania portu MySQL na zewnątrz.
